...
public final class GameplayControllerPlatformer extends GameplayControllerBase {
    ...
    private final IntArray keySpriteIndices;
    ...
    private final IntSet collectedKeySpriteIndices = new IntSet();
    ...
    public GameplayControllerPlatformer(
        ...
        keySpriteIndices = findSpriteIndicesByTypeOrName("key");
    ...
    private void collectTouchedItems() {
        collectTouchedGems();
        collectTouchedKeys();
    }
    ...
    private void collectTouchedGems() {
        ...
        // Disabled for Ocean Park: collecting all gems/coins should NOT finish the level.
        // The real win condition will be key + door + all players crossing.
        // if (gemSpriteIndices.size > 0 && collectedGemSpriteIndices.size >= gemSpriteIndices.size) {
        //     triggerWin();
        // }
    }

    private void collectTouchedKeys() {
        if (keySpriteIndices.size <= 0) {
            return;
        }

        for (int i = 0; i < keySpriteIndices.size; i++) {
            int spriteIndex = keySpriteIndices.get(i);
            if (collectedKeySpriteIndices.contains(spriteIndex)) {
                continue;
            }
            if (spriteIndex < 0 || spriteIndex >= spriteRuntimeStates.size) {
                continue;
            ...
            if (spritesOverlapByHitBoxes(
                ...
                collectedKeySpriteIndices.add(spriteIndex);
                hasKey = true;
                setSpriteVisible(spriteIndex, false);
            }
        }
